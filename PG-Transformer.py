...
class Model(nn.Module):
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec,
        ...
        ######################################################################################################################
        #zmh adding: 定义最相关的一列作为物理引导项,第一列为电流，第二列为功率，第三列为切削力
        boot_item = x_raw[:, :, 1]
        ...
        enc_out, attns = self.encoder(enc_out, attn_mask=enc_self_mask, tau=tau, delta=delta, boot_item=boot_item)

        dec_out = self.dec_embedding(x_dec_new, x_mark_dec)
        dec_out = self.decoder(dec_out, enc_out, x_mask=dec_self_mask, cross_mask=dec_enc_mask, tau=tau, delta=delta, boot_item=boot_item)
